...
export function filteredViews() {
  document.addEventListener("click", (e) => {
    ...
    if (
      e.target.closest("#projects-menu") &&
      e.target.classList[0] !== "add-project" &&
      e.target.id !== "projects-menu"
    ) {
      let projectTasks = Task.tasks.filter(
        (task) => task.project === tab && !task.completed
      );
      addHighlight(e.target);
      clearDOM(allCards);
