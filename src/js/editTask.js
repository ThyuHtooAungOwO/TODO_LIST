...
export function editTask() {
  ...
  editTaskForm.addEventListener("submit", (e) => {
    ...
    if (newSubtask !== "") {
      ...
      addSubtasks(
        newSubtask,
        currentTaskCard.querySelector(".tasks-container")
      );
    }

    // remove subtasks from object and corresponding card
    let removedSubtask = document
      .getElementById("remove-subtask")
      .value.split(",");
    if (removedSubtask[0] !== "") {
      for (const subtask of removedSubtask) {
        const subtaskIndex = taskObject.subtasks.findIndex(
          (obj) => obj.title === subtask.trim()
        );
